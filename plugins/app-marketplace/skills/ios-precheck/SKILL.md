...
```bash
bash --version && test -f precheck-ios.sh && echo "Ready"
```
...
**If the precheck script is not found**, ensure you are in the project root directory where `precheck-ios.sh` is located. The script must be executable:
```bash
chmod +x precheck-ios.sh
```
